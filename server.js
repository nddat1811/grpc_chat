...
const join = (call, callback) => {
  ...
  if (!userExiist) {
    ...
    printMessage(
      "login",
      `${user.name} joined, users: [${userInRoom
        .map((e) => e.user.name)
        .join(", ")}]`
    );
    ...
    userInRoom.forEach((e) => {
      e.notificate?.write({
        code: 0,
        msg: `${user.name} joined, users: [${userInRoom
          .map((e) => e.user.name)
          .join(", ")}]`,
      });
    });
...
const clearUser = (user) => {
  ...
  if (userIndex != -1) {
    ...
    printMessage(
      "logout",
      `${user.name} left, users: [${userInRoom
        .map((e) => e.user.name)
        .join(", ")}]`
    );
    ...
    userInRoom.forEach((e) => {
      e.notificate.write({
        code: 0,
        msg: `${user.name} left, users: [${userInRoom
          .map((e) => e.user.name)
          .join(", ")}]`,
      });
    });
...
const sendMsg = (call, callback) => {
  ...
  printMessage(
    "message",
    `${data.msg} (from: ${data.from}, uuid: ${data.uuid})`
  );
  ...
  switch (true) {
    ...
    default:
      ...
      sendNotificate(msg.from, {
        code: 0,
        msg: `${user.name} like your msg uuid=${msg.uuid} (${
          msg.likeList.length
        } likes${
      ...
      printMessage(
        "like",
        `${user.name} like msg uuid=${msg.uuid} (${msg.likeList.length} likes)`
      );
      break;
  }
  if (msgError != "") {
